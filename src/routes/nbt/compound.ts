...
function readCompound(buffer, offset, typeArgs, rootNode, history: any[]) {
	const results = {
		value: {},
		size: 0
	}
	while (true) {
		const typ = this.read(buffer, offset, 'i8', rootNode, history)
		if (typ.value === 0) {
			offset += typ.size
		...
		const readResults = this.read(buffer, offset, 'nbt', rootNode, history)
