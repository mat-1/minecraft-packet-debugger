...
function createProto(type) {
	...
	compiler.addTypes(optional.interpret)
	let proto = beNbtJson
...
export function parseUncompressed(
	data,
	proto: keyof typeof protos | boolean = 'big',
	options = {}
) {
	if (proto === true) proto = 'little'

	protos[proto as keyof typeof protos].setVariable('noArraySizeCheck', options.noArraySizeCheck)
...
const hasBedrockLevelHeader = (data) => data[1] === 0 && data[2] === 0 && data[3] === 0
...
async function parse(data, format, callback) {
	...
	const verifyEOF = ({ buffer, size }) => {
		...
		if (readLen < bufferLen && !nextNbtTag) {
			throw new Error(
				`Unexpected EOF at ${readLen}: still have ${bufferLen - readLen} bytes to read !`
			)
		}
	}
...
function equal(nbt1, nbt2) {
	...
	if (nbt1.type === 'longArray') {
		...
		for (let i = 0; i < nbt1.value.length; i++) {
			if (nbt1.value[i][0] !== nbt2.value[i][0] || nbt1.value[i][1] !== nbt2.value[i][1])
				return false
		}
