...
export function getCount(buffer: Buffer, offset: number, { count, countType }: { count: number, countType: string }, rootNode: any, history: any[]) {
    let c = 0
    let size = 0
    if (typeof count === 'number') { c = count } else if (typeof count !== 'undefined') {
        c = getField(count, rootNode)
    } else if (typeof countType !== 'undefined') {
        ({ size, value: c } = tryDoc(() => this.read(buffer, offset, getFieldInfo(countType), rootNode, history), '$count'))
...
export function calcCount(len: number, { count, countType }: { count: number, countType: string }, rootNode) {
